...
export class SeedDefaultDepartments1775526400000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      INSERT INTO departments (id, name, short_code, description, is_active, created_at, updated_at)
      VALUES
        (uuid_generate_v4(), 'Direct Cash Transfer', 'DCT', 'Disbursement to verified beneficiaries', true, NOW(), NOW()),
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      WHERE short_code IN ('DCT', 'OPS', 'M&E', 'LOG', 'OFF');
    `);
  }
}
